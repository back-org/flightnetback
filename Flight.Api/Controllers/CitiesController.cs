...
public class CitiesController : ParentController
{
    ...
    public async Task<ActionResult<CityDto>> Get([FromRoute] int id)
    {
        ...
        if (result is null)
        {
            return NotFoundResponse(
                "Ville introuvable.",
                $"Aucune ville n'a été trouvée avec l'identifiant {id}.");
        }
    ...
    public async Task<ActionResult<CityDto>> Create([FromBody] CityDto dto)
    {
        ...
        var result = await Mediator.Send(
            new CreateCityCommand(dto, User.Identity?.Name ?? "system"));
    ...
    public async Task<ActionResult<CityDto>> Put([FromBody] CityDto dto)
    {
        ...
        var result = await Mediator.Send(
            new UpdateCityCommand(dto.Id, dto, User.Identity?.Name ?? "system"));

        if (result is null)
        {
            return NotFoundResponse(
                "Ville introuvable.",
                $"Aucune ville n'a été trouvée avec l'identifiant {dto.Id}.");
        }
    ...
    public async Task<ActionResult> Delete([FromRoute] int id)
    {
        var success = await Mediator.Send(
            new DeleteCityCommand(id, User.Identity?.Name ?? "system"));
